 ...
 *
 * A namespace NS that has the same name as a class OC adds all its declarations
 * to OC. Currently, only class and enum declarations inside NS are supported.
 * The declarations are renamed and hoisted to the file level. A type alias
 * property for each declaration in NS is added to class OC. The namespace is
 * then eliminated so that tsickle does not generate an iife.
 *
 * Example:
 * class Outer { }
 * namespace Outer {
 ...
 * /** const * / Outer.InnerClass = Outer$InnerClass;
 * /** const * / Outer.InnerEnum = Outer$InnerEnum;
 *
 */
    ...
    return (sourceFile: ts.SourceFile): ts.SourceFile => {
      ...
      function transformNamespace(
        ...
        function transformInnerClass(classDecl: ts.ClassDeclaration) {
          ...
          const className = getIdentifierText(classDecl.name as ts.Identifier);
        ...
        function transformInnerEnum(enumDecl: ts.EnumDeclaration) {
          ...
          const enumName = getIdentifierText(enumDecl.name as ts.Identifier);
